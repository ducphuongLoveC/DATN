...
const CourseForm: React.FC<CourseFormProps> = ({ datas, onSubmit }) => {
  ...
  const Row = memo(({ row, currentModuleIndex }: any) => {
    ...
    const handleToggleActiveModule = (event: React.ChangeEvent<HTMLInputElement>) => {
      let cloneModules: Module[] = [...modules];
      cloneModules[currentModuleIndex].isActive = event.target.checked;
      setModules(cloneModules);
    };
    const handleToggleActiveResource = (index: number, checked: boolean) => {
      let cloneModules: Module[] = [...modules];
      cloneModules[currentModuleIndex].resources[index].isActive = checked;
      setModules(cloneModules);
    };
    ...
    return (
      <>
        <TableRow sx={{ boxShadow: 'var(--main-box-shadow)' }}>
          ...
          <TableCell align="right">
            <Switch onChange={handleToggleActiveModule} checked={row.isActive} />
          </TableCell>
                  ...
                  <TableBody>
                    {row.resources.map((resource: Resource, index: number) => (
                      <TableRow key={resource.title}>
                        ...
                        <TableCell align="right">
                          <Switch
                            onChange={(event) => handleToggleActiveResource(index, event.target.checked)}
          ...
          <Table sx={{ minWidth: 650 }} aria-label="collapsible table">
            ...
            <TableBody>
              {modules.map((module: Module, index: number) => (
                <Row key={module.title} row={module} currentModuleIndex={index} />
              ))}
            </TableBody>
          </Table>
